...
├── core/
│   ├── brain-router.ts    # BrainRouter — per-message routing between Grok and Claude brains
│   ├── claude-brain.ts    # ClaudeBrain — Claude Code CLI subprocess wrapper (spawn, timeout, retry)
...
│   ├── studio/            # Studio module — Forge HQ data integration
│   │   ├── types.ts        # DispatchRecord, AgentInfo, ProjectInfo, AGENT_REGISTRY, PROJECT_REGISTRY
│   │   ├── hq-utils.ts     # findHqDir(), readJsonSafe(), matchesQuery(), getProjectSlugs()
│   │   ├── query-studio.ts # studio.query tool — keyword search across HQ data
│   │   ├── update-studio.ts # studio.update tool — write recommendations, activity, features
│   │   ├── dispatch-agent.ts # studio.dispatch_agent tool — async Claude Code agent subprocess
│   │   └── index.ts        # FridayModule barrel — registers 3 tools
...
| **Modules** | `src/modules/` | Auto-loaded tool/protocol bundles. Shared validation in `validation.ts`. 9 modules: filesystem, git, docker, code-exec, web-fetch, notify, forge, gmail, studio. |
...
| **BrainRouter** | `src/core/brain-router.ts` | Pure routing — prefix > mode > voice > keyword > continuity > length > default. No side effects. |
| **ClaudeBrain** | `src/core/claude-brain.ts` | `claude -p` subprocess. Timeout via Promise.race, retry on empty, truncate on overflow. |
| **Studio Module** | `src/modules/studio/` | 3 tools: query_studio (read HQ), update_studio (write HQ), dispatch_agent (async subprocess). |

**Boot order:** SignalBus → ClearanceManager → AuditLogger → NotificationManager → ProtocolRegistry → DirectiveStore/Engine → Memory → SmartsStore → Sensorium → Genesis → Vox → BrainRouter + ClaudeBrain → Cortex → Recall Tool → Arc Rhythm → Modules → `session:start`
...
- **Dual-brain routing**: `chatWithRouting()` on Cortex checks BrainRouter, falls back to `chatStream()` when no router configured. Voice pipeline queries BrainRouter first to avoid double-inference, then either `chatStreamVoice()` (Grok) or `chatWithRouting()` + Grok TTS inject (Claude).
- **Claude→Grok TTS passthrough**: When BrainRouter sends voice to Claude, the response text is injected into the Grok WebSocket via `conversation.item.create` + `response.create` with `modalities: ["audio"]` for speech synthesis.
- **Studio module tools**: `studio.query` reads HQ data files, `studio.update` writes recommendations/activity/features, `studio.dispatch_agent` spawns `claude -p` with agent skill content as system prompt.

## Testing

- 1164 tests across 108 files (as of 2026-03-16)
